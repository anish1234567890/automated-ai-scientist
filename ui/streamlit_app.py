...
with st.sidebar:
    ...
    st.divider()
    page = st.radio("Navigate", ["🚀 Run Experiment", "📓 Lab Notebook"],
                    label_visibility="collapsed")
    st.divider()
    st.markdown("**Supervised (21 models)**")
    st.caption("RF, XGB, LightGBM, Ridge, Lasso, SVM, KNN ...")
    st.caption("✅ 5-fold CV  ✅ Ensemble  ✅ Feature Engineering")
    st.markdown("**Unsupervised (6 algorithms)**")
    st.caption("K-Means, DBSCAN, GMM, Agglomerative, Isolation Forest, LOF")
    st.caption("✅ Multi-metric Optuna  ✅ PCA scatter  ✅ Anomaly detection")
    st.divider()
    st.markdown("**Tech Stack**")
    st.caption("LLM: Llama 3.3 70B (Groq)")
    st.caption("Tuning: Optuna TPE + MedianPruner")
    st.caption("CV: 5-fold cross-validation")
...
if "Run" in page:
    ...
    st.markdown(
        "*LLM selects algorithms → Optuna tunes → 5-fold CV evaluates → "
        "Ensemble combines → Science happens.*"
    )
    st.divider()
    ...
    with col1:
        ...
        st.caption(
            "**Supervised**: CSV must have a `target` column.  "
            "**Unsupervised**: any CSV or use clustering keywords in prompt."
        )
        ...
        if uploaded:
            ...
            if has_target:
                st.success(
                    f"✅ {df_preview.shape[0]} rows × {df_preview.shape[1]} cols"
                    f" — **target column found** (supervised ready)"
                )
            else:
                st.info(
                    f"ℹ️ {df_preview.shape[0]} rows × {df_preview.shape[1]} cols"
                    f" — **no target column** (will run unsupervised)"
                )
        ...
        n_trials = st.slider(
            "Optuna trials per algorithm", min_value=10, max_value=50,
            value=25, step=5,
            help="More trials = better hyperparameter tuning but slower runtime.",
        )
    ...
    if run_btn:
        ...
        with st.spinner("Running experiment..."):
            output = run_ai_scientist(
                user_prompt, progress_callback=update_status, n_trials=n_trials
            )
        ...
        if mode == "supervised":
            ...
            with m4:
                if ensemble and not ensemble.get("error"):
                    label = "Ensemble Acc" if task == "classification" else "Ensemble RMSE"
                    st.metric(label, f"{ensemble['cv_score']:.4f}",
                              delta=f"Top-{len(ensemble.get('models_used', []))}")
            with m5:
                st.metric("Dataset", f"{shape[0]}×{shape[1]}" if shape else "N/A")

            st.divider()
            tab1, tab2, tab3, tab4, tab5 = st.tabs([
                "🏆 Leaderboard", "⚙️ Parameters", "🔬 AI Insights",
                "💻 Final Code", "📄 Report",
            ])

            # ── Tab 1: Leaderboard ────────────────────────────────
            with tab1:
                st.subheader(f"🏆 Model Leaderboard — {metric} (5-fold CV)")
                st.caption(
                    "Scores are 5-fold cross-validation averages. "
                    "Pipeline: PolynomialFeatures (interaction) → SelectKBest (30%) → Model."
                )
                if models_data:
                    best = models_data[0]
                    st.markdown(
                        f'<div class="best-model">🥇 <strong>Best Model: {best["name"]}</strong>'
                        f' &nbsp;|&nbsp; {metric} (CV-5): <strong>{best["score"]:.4f}</strong>'
                        f'</div>',
                        unsafe_allow_html=True,
                    )
                    ...
                    # Ensemble banner
                    if ensemble and not ensemble.get("error"):
                        used = ", ".join(ensemble.get("models_used", []))
                        st.markdown(
                            f'<div class="ensemble-card">🤝 <strong>Ensemble (Top-3 Voting)</strong>'
                            f' &nbsp;|&nbsp; {metric}: <strong>{ensemble["cv_score"]:.4f}</strong>'
                            f' &nbsp;|&nbsp; Models: {used}</div>',
                            unsafe_allow_html=True,
                        )
            ...
            with tab2:
                ...
                st.caption(
                    f"Optuna TPE sampler + MedianPruner | "
                    f"{results.get('n_trials_per_model', 25)} trials per model"
                )
                for i, m in enumerate(models_data):
                    medal = ["🥇", "🥈", "🥉"][i] if i < 3 else f"#{i+1}"
                    with st.expander(
                        f"{medal} {m['name']}  |  {metric}: {m.get('score', 0):.4f}",
                        expanded=(i == 0),
                    ):
                        ca, cb = st.columns(2)
                        with ca:
                        ...
                        with cb:
                            ...
                            if params:
                                st.markdown("**Best Hyperparameters (Optuna TPE):**")
                                st.dataframe(
                                    pd.DataFrame([
                                ...
                                st.info("No tunable hyperparameters (e.g. Linear Regression)")

                # Ensemble breakdown
                if ensemble and not ensemble.get("error"):
                    st.divider()
                    st.subheader("🤝 Ensemble Details")
                    st.markdown(f"**Models combined:** {', '.join(ensemble.get('models_used', []))}")
                    st.markdown(f"**Voting strategy:** `{ensemble.get('voting', 'N/A')}`")
                    st.markdown(f"**CV {metric}:** `{ensemble.get('cv_score', 'N/A')}`")
                    st.caption("Ensemble uses the same Pipeline (PolyFeatures → SelectKBest → Voting) with 5-fold CV.")
            ...
            with tab3:
                ...
                st.caption(
                    "Dataset-aware analysis: the LLM receives row count, column count, "
                    "null values, and target type before selecting models."
                )
                st.info(insight)

            # ── Tab 4: Final Code ─────────────────────────────────
            with tab4:
                st.subheader("💻 Final ML Code — CV Pipeline + Ensemble")
                st.caption(
                    "Fully runnable Python. Uses exact best params found by Optuna. "
                    "Includes Pipeline (PolyFeatures → SelectKBest → Model) and Voting Ensemble."
                )
                if final_code:
                    st.code(final_code, language="python")
                    st.download_button(
                        "⬇️ Download final_ml_code.py", data=final_code,
            ...
            with tab5:
                ...
                if report_path and os.path.exists(report_path):
                    ...
                    st.download_button(
                        f"⬇️ Download Report ({ext.upper()})", data=file_bytes,
                        file_name=f"report{ext}", mime=mime,
        ...
        # ══════════════════════════════════════════════════════════
            ...
            clustering    = [c for c in all_cls
                             ...
                             and c["name"].lower() not in anomaly_names]
            anomalies     = [c for c in all_cls
                             if c.get("silhouette") is not None
                             and c["name"].lower() in anomaly_names]
            ...
            with m3:
                if best_overall:
                    st.metric("Best Silhouette", f"{best_overall['silhouette']:.4f}",
            ...
            st.divider()
            tab1, tab2, tab3, tab4, tab5 = st.tabs([
                "🔵 Clusters", "🔴 Anomalies", "🔬 AI Insights",
                "💻 Final Code", "📄 Report",
            ])
            ...
            with tab1:
                ...
                st.caption(
                    "Sorted by composite score: 0.5×Silhouette − 0.3×Davies-Bouldin + 0.2×Calinski-Harabasz. "
                    "Silhouette range: −1 (wrong) to +1 (perfect)."
                )
                ...
                if clustering:
                    ...
                    st.markdown(
                        ...
                        f' &nbsp;|&nbsp; Clusters found: <strong>{best_c.get("n_clusters_found","?")}</strong>'
                        f'</div>',
                        unsafe_allow_html=True,
                    )
                    ...
                    if pca_coords and best_labels:
                        ...
                        st.caption(
                            f"2D PCA projection — colored by {best_c['name']} assignments"
                        )
                        ...
                        if pca_var:
                            tv = round(sum(pca_var) * 100, 1)
                            p1 = round(pca_var[0] * 100, 1)
                            p2 = round(pca_var[1] * 100, 1) if len(pca_var) > 1 else "N/A"
                            st.caption(
                                f"PCA explains {tv}% total variance  |  "
                                f"PC1: {p1}%  |  PC2: {p2}%"
                            )
                    ...
                    for i, c in enumerate(clustering):
                        ...
                        with st.expander(
                            f"{medal} {c['name']}  |  "
                            f"Silhouette: {c.get('silhouette', 0):.4f}  |  "
            ...
            with tab2:
                ...
                st.caption(
                    "Isolation Forest and Local Outlier Factor label each point: "
                    "0 = Normal, 1 = Anomaly."
                )
                if anomalies:
                    for a in anomalies:
                        ...
                        st.markdown(
                            f'<div style="background:#fff0f0;border-left:4px solid #ef4444;'
                        ...
                        if pca_coords and best_labels:
                            ...
                            adf["Type"] = ["Anomaly" if l == 1 else "Normal"
                            ...
                            try:
                                ...
                                adf["ColorCode"] = [1 if t == "Anomaly" else 0
                                                    for t in adf["Type"]]
                                st.scatter_chart(adf, x="PC1", y="PC2", color="ColorCode")
                        st.divider()
                else:
                    st.info(
                        "No anomaly detection algorithms were run. "
                        "Add **'Isolation Forest'** or **'Local Outlier Factor'** to your prompt — "
                        "e.g. *'detect anomalies using isolation forest'*"
                    )
            ...
            with tab3:
                st.subheader("🔬 Scientific Analysis by AI Researcher Agent")
                st.caption(
                    "Analysis covers: best algorithm reason, hyperparameter interpretation, "
                    "PCA dimensionality, recommendation, and density structure."
                )
                st.info(insight)
            ...
            with tab4:
                ...
                st.caption(
                    "Fully runnable Python with Optuna-tuned parameters, "
                    "StandardScaler, PCA visualization, and silhouette evaluation."
                )
                if final_code:
                    st.code(final_code, language="python")
                    st.download_button(
                        "⬇️ Download final_unsupervised_code.py", data=final_code,
...
# ══════════════════════════════════════════════════════════════════
    ...
    st.markdown("*Every experiment stored permanently in SQLite — supervised & unsupervised.*")
    st.divider()
    ...
    if not experiments:
        ...
        for exp in experiments:
            ...
            score_str = format_score(exp.get("best_score"))
            ...
            with st.expander(f"{icon} [{exp['timestamp']}]  {label}  →  {badge}"):
                ...
                with col1:
                    ...
                    # Show ensemble if present
                    exp_results = exp.get("results", {})
                    ens = exp_results.get("ensemble", {})
                    if ens and not ens.get("error"):
                        metric_label = "Accuracy" if task == "classification" else "RMSE"
                        st.markdown(
                            f"**🤝 Ensemble CV {metric_label}:** `{ens.get('cv_score', 'N/A')}`  "
                            f"*(Top-3 Voting)*"
                        )

                with col2:
                    if mode == "unsupervised":
                        cls_data = [c for c in exp_results.get("clustering", [])
                                    if c.get("silhouette") is not None]
                        if cls_data:
                            rows = []
                            for c in cls_data:
                                params = c.get("best_params", {})
                                n_anom = (params.get("anomalies_found")
                                          or params.get("outliers_found"))
                                row = {
                                    "Algorithm":      c["name"],
                                    "Silhouette ↑":   round(c.get("silhouette") or 0, 4),
                        ...
                        metric     = "Accuracy" if task == "classification" else "RMSE"
                        models_data = [m for m in exp_results.get("models", [])
                                       if m.get("score") is not None]
                        if models_data:
                            rows = [{"Model": m["name"], f"{metric} (CV-5)": round(m["score"], 4)}
                ...
                if final_code:
                    fname = (
                        f"{'unsupervised' if mode == 'unsupervised' else 'ml'}"
